...
def Plano(ancho, alto, pantalla):
    ...
    inix = (0, alto / 2)
    finx = (ancho, alto / 2)
    # linea horizontal del plano cartesiano
    pygame.draw.line(pantalla, ROJO, inix, finx, 1)
    # linea vertical del plano cartesiano
    pygame.draw.line(pantalla, ROJO, iniy, finy, 1)
    pygame.display.flip()
...
def rotacion(punto, teta):
    x = punto[0]
    y = punto[1]
    xr = (x * math.cos(math.radians(teta))) - \
        (y * math.sin(math.radians(teta)))
    yr = (x * math.sin(math.radians(teta))) + \
        (y * math.cos(math.radians(teta)))
    return (xr, yr)
